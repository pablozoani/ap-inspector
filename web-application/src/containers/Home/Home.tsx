...
const Home = () => {
    ...
    const currentCategoryId = useSelector((state: RootState) => state.home.currentCategoryId);
    const currentVendorId = useSelector((state: RootState) => state.home.currentVendorId);
    const currentSearchKey = useSelector((state: RootState) => state.home.currentSearchKey);
    ...
    }, [pageSize, currentSortType, currentVendorId, currentCategoryId, currentSearchKey]);
    ...
    const cards = products.map(product => {
        ...
        return (
            <Card title={product.name} key={product.id}>
                <small>{vendor ? `by ${vendor.firstName} ${vendor.lastName}` : null}</small>
                <div
                    style={{
                        display: "flex",
                        flexFlow: "row",
    ...
    const vendorGroup = vendors.map(vendor => {
        return (
            <option key={vendor.id} value={`vendor ${vendor.id}`}>{`${vendor.firstName} ${vendor.lastName}`}</option>
        );
    });
        ...
        <>
            ...
            >
                ...
                <Menu
                    ...
                    menuItems={[
                        <MenuItem key="abcd" clicked={() => dispatch(setScene(Scenes.ADD_PRODUCT))}>
                            Add new product
                        </MenuItem>
                    ]}
                />
            ...
            >
                <input
                    ...
                    placeholder="Search..."
                    autoFocus
                    onChange={event => dispatch(setCurrentSearchKey(event.currentTarget.value))}
                    onKeyPress={event => {
                        if (event.key === "Enter") {
                ...
                <select onChange={event => sortBy(event.currentTarget.value)} className={classes.Select}>
